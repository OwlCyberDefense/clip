...
If you haven't read the PCI Firefox requirements, don't run this script.  

Go read the PCI guidance so you can understand what is happening.

############################################
2. Firefox lockdown overview

To lock down Firefox you must first have the settings you want to adjust.  
This can be obtained from the PCI website.  
...
lockdown content we call the config file mozilla-aqueduct-pci.cfg.  

The mozilla-aqueduct-pci.cfg file must reside in the following directory:
...
The following lines point firefox to the mozilla-aqueduct-PCI.cfg file:

	pref("general.config.filename", "mozilla-aqueduct-PCI.cfg");
...
One of the requirements for the Firefox PCI is to install the DoD root Certs.  At
this time the Aqueduct Firefox lockdown script does not do this.  

This is extremely complicated to accomplish since the PCI is vague.  
...
For more information on which checks are not addressed, see mozilla-aqueduct-PCI.cfg
