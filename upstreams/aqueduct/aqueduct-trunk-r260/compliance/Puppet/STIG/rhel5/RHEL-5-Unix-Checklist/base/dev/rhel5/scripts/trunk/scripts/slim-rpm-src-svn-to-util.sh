  ...
  for rel in ${RELEASEPHASE};do
    for os in ${OPERATINGSYSTEM};do
      # for arch in ${ARCHITECTURE};do
            ...
            if [ ! -d "$HOMEDIR/$rel/$os/$RPMSOURCE" ];then
                          # ... then we do a check check to see if the svn repo structure exists...
              svn --non-interactive list $SVNSERVER/$rel/$os/$RPMSOURCE
              ...
              svn --quiet --non-interactive co $SVNSERVER/$rel/$os/$RPMSOURCE $HOMEDIR/$rel/$os/$RPMSOURCE \
                  || error_exit "Line $LINENO: Could not run $SVNSERVER/$rel/$os/$RPMSOURCE $HOMEDIR/$rel/$os/$RPMSOURCE"
                        fi
                fi
      done
    done
    ...
    for os in ${OPERATINGSYSTEM};do
      # for arch in ${ARCHITECTURE};do
            ...
            if [ -d "$HOMEDIR/$rel/$os/$RPMSOURCE" ];then
              svn --non-interactive list $SVNSERVER/$rel/$os/$RPMSOURCE
                    if [ $? == "0" ];then
              svn --quiet --non-interactive update $SVNSERVER/$rel/$os/$RPMSOURCE $HOMEDIR/$rel/$os/$RPMSOURCE \
                  || error_exit "Line $LINENO: Could not run $SVNSERVER/$rel/$os/$RPMSOURCE $HOMEDIR/$rel/$os/$RPMSOURCE"
                    fi
                fi
          done
