...
#Group Title: The root account has an imbedded . or ::, or : as
#Rule ID: SV-776r7_rule
#Severity: CAT II
#Rule Version (STIG-ID): GEN000940
#Rule Title: The root account's executable search path must be the vendor default and must contain only absolute paths.
#
...
#Fix Text: Edit the root user's local initialization files ~.profile,~.bashrc (assumimg root shell is bash). Change any found PATH variable settings to the vendor's default path for the root user. Remove any empty path entries or references to relative paths.    
#######################DISA INFORMATION###############################
