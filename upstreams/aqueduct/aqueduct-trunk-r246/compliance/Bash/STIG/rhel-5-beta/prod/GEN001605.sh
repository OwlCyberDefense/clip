...
#Rule Title: Run control scripts' library search paths must contain 
#only absolute paths.
#
...
#directory or other relative paths, libraries in these directories may 
#be loaded instead of system libraries. This variable is formatted as 
#a colon-separated list of directories. If there is an empty entry, 
#such as a leading or trailing colon, or two consecutive colons, this 
#is interpreted as the current working directory. Paths starting with a 
...
#entry from the library search path variable.   
#######################DISA INFORMATION###############################
