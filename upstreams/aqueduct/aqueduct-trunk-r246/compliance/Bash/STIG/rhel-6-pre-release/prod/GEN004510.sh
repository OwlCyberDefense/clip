...
CRIT_MAIL_LOG=$( egrep "(\*.crit|mail\.[^n][^/]*)" ${SYSLOGFILE} | sed 's/^[^/]*//' )
#Start-Lockdown
for LOGFILE in $(/bin/echo ${CRIT_MAIL_LOG})
do
    if [[ -f "${LOGFILE}" ]]
    then
        ACLOUT=`getfacl --skip-base ${LOGFILE} 2>/dev/null`;
        if [ "$ACLOUT" != "" ]
            then
            setfacl --remove-all ${LOGFILE}
        fi
    fi
done
