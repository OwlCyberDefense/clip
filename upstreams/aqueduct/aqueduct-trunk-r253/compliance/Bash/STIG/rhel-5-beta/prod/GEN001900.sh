...
#Rule Title: All local initialization files' executable search paths must 
#contain only absolute paths.
#
#Vulnerability Discussion: The executable search path (typically the PATH 
#environment variable) contains a list of directories for the shell to 
#search to find executables. If this path includes the current working 
#directory or other relative paths, executables in these directories may be 
#executed instead of system commands. This variable is formatted as a 
#colon-separated list of directories. If there is an empty entry, such as a leading 
#or trailing colon, or two consecutive colons, this is interpreted as the 
#current working directory. Paths starting with a slash (/) are absolute paths.
#
#Responsibility: System Administrator
#IAControls: ECCD-1, ECCD-2
#
#Check Content: 
#Check that local initialization files have executable search path containing 
#only absolute paths.
#
#Procedure:
...
#path entry from the executable search path variable.   
#######################DISA INFORMATION###############################
