...
# on 30-dec-2011 to set the banner_file option to point to /etc/issue
...
#Rule Title: The FTPS/FTP service on the system must be configured 
#with the Department of Defense (DoD) login banner.
#
#Vulnerability Discussion: Failure to display the logon banner 
#prior to a logon attempt will negate legal proceedings resulting 
#from unauthorized access to system resources.
#
#Note: SFTP and FTPS are encrypted alternatives to FTP that should 
#be used in place of FTP. SFTP is implemented by the SSH service 
#and uses its banner configuration.
#
...
#FTP to the system.
# ftp localhost
#Check for either of the following login banners based on the 
#character limitations imposed by the system. An exact match 
#is required. If one of these banners is not displayed, this 
#is a finding. If the system does not run the FTP service, 
...
#
#Fix Text: Edit /etc/banners/vsftpd and add one of the DoD 
#login banners (based on the character limitations imposed 
#by the system).
#
#DoD Login Banners:
#
