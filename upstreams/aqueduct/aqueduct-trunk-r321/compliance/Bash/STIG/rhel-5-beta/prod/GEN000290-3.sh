...
#######################DISA INFORMATION###############################
#Group ID (Vulid): V-4269-3
#Group Title: Unnecessary Accounts, Gopher.
#Rule ID: SV-4269-3r4_rule
#Severity: CAT II
...
#
#Check Content: 
#Check the system the unnecessary "gopher" accounts.
#
#Procedure:
# grep ^gopher /etc/passwd
#If this account exists, it is a finding.
#
#Fix Text: Remove the "gopher" account from the /etc/password file 
#before connecting a system to the network.   
#######################DISA INFORMATION###############################

#Global Variables#
PDI=GEN000290-3
GOPHERACCOUNT=`grep -c '^gopher' /etc/passwd`

#Start-Lockdown

if [ $GOPHERACCOUNT -ne 0 ]
  then
    userdel gopher 
fi
