...
#
#Check Content: 
#For each user in /etc/passwd with a home directory check the ownership of 
#local initialization files.
#
#Procedure:
# files=" .login .cschrc .logout .profile .bash_profile .bashrc .bash_logout 
#.env .dtprofile .dispatch .emacs .exrc";for pwline in `cut -d: -f1,6 /etc/passwd`
#; do homedir=`echo ${pwline}|cut -d: -f2`;username=`echo ${pwline} | cut -d: 
#-f1`;for thisone in $files; do stat -c %U:%n ${homedir}/${thisone} 2>null|
#egrep -v "^(root|${username}):"; done;done
#and:
# for pwline in `cut -d: -f1,6 /etc/passwd`; do homedir=`echo ${pwline}|cut 
#-d: -f2`;username=`echo ${pwline} | cut -d: -f1`;find ${homedir}/.dt ! -fstype 
#nfs ! -user ${username} -exec stat -c %U:%n {} \; 2>null; done
#
#If local initialization files are not owned by the home directory user, 
#this is a finding.
#
#Fix Text: Change the ownership of the startup and login files in the user’s 
#directory to the user or root, as appropriate. Examine each user’s home 
#directory and verify that all filenames that begin with "." are owned by 
#the owner of the directory or root. If they are not, use the chown command 
#to change the owner to the user and research the reasons why the owners 
#were not assigned as required.
#
#Procedure:
