...
# Group Title: GEN000620
# Rule ID: SV-37281r1_rule
# Severity: medium
...
do
  ...
  then
    ...
    then
      ...
      sed -i -e 's/\(^password.*-auth-ac\)/password      required      pam_cracklib.so dcredit=-1\n\1/' /etc/pam.d/${PFPREFIX}-auth-local
    else
      #Add the new argument if the line exists and the config does not
      egrep '^[^#]?password.*pam_cracklib' /etc/pam.d/${PFPREFIX}-auth-local | grep dcredit=-1 > /dev/null
      if [ $? != 0 ]
      then
        sed -i -e 's/\(^password.*pam_cracklib.*$\)/\1 dcredit=-1/' /etc/pam.d/${PFPREFIX}-auth-local
      fi
    fi
  fi
done
