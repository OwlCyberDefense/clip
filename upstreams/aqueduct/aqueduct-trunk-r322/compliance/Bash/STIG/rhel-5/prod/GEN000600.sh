...
#######################DISA INFORMATION##################################
# Group ID (Vulid): V-11948
# Group Title: GEN000600
# Rule ID: SV-41826r1_rule
# Severity: medium
# Rule Version (STIG-ID): GEN000600
...
do
  ...
  then
    ...
    then
      ...
      egrep '^[^#]?password.*pam_cracklib' /etc/pam.d/${PFPREFIX}-auth-local | grep ucredit=-1 > /dev/null
      if [ $? != 0 ]
      then
        sed -i -e 's/\(^password.*pam_cracklib.*$\)/\1 ucredit=-1/' /etc/pam.d/${PFPREFIX}-auth-local
      fi
    fi
  fi
done
