...
#contains a list of libraries for the dynamic linker to load before loading 
#the libraries required by the binary. If this list contains paths to 
#libraries relative to the current working directory, unintended libraries 
#may be preloaded. This variable is formatted as a space-separated list of 
#libraries. Paths starting with a slash (/) are absolute paths.
#
...
# grep -r LD_PRELOAD /etc/profile /etc/bashrc /etc/csh.login 
#/etc/csh.cshrc /etc/environment /etc/.login /etc/security/environ 
#/etc/profile.d/*
#This variable is formatted as a colon-separated list of paths. If there 
#is an empty entry, such as a leading or trailing colon, or two consecutive 
#colons, this is a finding. If an entry begins with a character other than 
#a slash (/) this is a relative path, this is a finding.
#
#Fix Text: Edit the global initialization file and remove the relative 
#path entry from the library preload variable.    
#######################DISA INFORMATION###############################
