    ...
    for os in ${OPERATINGSYSTEM};do
      for arch in ${ARCHITECTURE};do
            ...
            if [ ! -d "$HOMEDIR/$rel/$os/$TRUNKKICKSTART/$arch" ];then
              ...
              svn --non-interactive list $SVNSERVER/$rel/$os/$TRUNKKICKSTART/$arch
              ...
              svn --quiet --non-interactive co $SVNSERVER/$rel/$os/$TRUNKKICKSTART/$arch $HOMEDIR/$rel/$os/$TRUNKKICKSTART/$arch \
                  || error_exit "Line $LINENO: Could not run $SVNSERVER/$rel/$os/$TRUNKKICKSTART/$arch $HOMEDIR/$rel/$os/$TRUNKKICKSTART/$arch"
                        fi
                fi
      done
    done
    ...
    for os in ${OPERATINGSYSTEM};do
      for arch in ${ARCHITECTURE};do
            ...
            if [ -d "$HOMEDIR/$rel/$os/$TRUNKKICKSTART/$arch" ];then
              svn --non-interactive list $SVNSERVER/$rel/$os/$TRUNKKICKSTART/$arch
                    if [ $? == "0" ];then
              svn --quiet --non-interactive update $SVNSERVER/$rel/$os/$TRUNKKICKSTART/$arch $HOMEDIR/$rel/$os/$TRUNKKICKSTART/$arch \
                  || error_exit "Line $LINENO: Could not run $SVNSERVER/$rel/$os/$TRUNKKICKSTART/$arch $HOMEDIR/$rel/$os/$TRUNKKICKSTART/$arch"
                    fi
                fi
          done
