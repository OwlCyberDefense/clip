...
#######################DISA INFORMATION###############################
#Group ID (Vulid): V-4269-1
#Group Title: Unnecessary Accounts, Games.
...
#the system and application accounts for applications not installed on the system.
#
#Responsibility: System Administrator
#IAControls: IAAC-1
#
#Check Content: 
#Check the system the unnecessary "games" accounts.
#
#Procedure:
# grep ^games /etc/passwd
#If this account exists, it is a finding.
#
#Fix Text: Remove the "games" account from the /etc/password file 
#before connecting a system to the network.   
#######################DISA INFORMATION###############################

#Global Variables#
PDI=GEN000290-1
GAMESACCOUNT=`grep -c '^games' /etc/passwd`

#Start-Lockdown

if [ $GAMESACCOUNT -ne 0 ]
  then
    userdel games
fi
