...
#anyone running these files to be temporarily assigned the UID of the file. 
#While many system files depend on these attributes for proper operation, 
#security problems can result if setuid is assigned to programs that allow 
#reading and writing of files, or shell escapes. Only default vendor-supplied 
#executables should have the setuid bit set.
#
#Responsibility: System Administrator
#IAControls: ECPA-1
#
#Check Content: 
#Files with the setuid bit set will allow anyone running these files to be 
#temporarily assigned the user or group ID of the file. If an executable 
#with setuid allows shell escapes, the user can operate on the system with 
#the effective permission rights of the user or group owner.
#
#List all setuid files on the system.
#Procedure:
# find / -perm -4000 -exec ls -l {} \; | more
#
#Note: Executing these commands may result in large listings of files; 
#the output may be redirected to a file for easier analysis.
#
#Ask the SA or IAO if files with the suid bit set have been documented. 
#If any undocumented file has its suid bit set, this is a finding.
#
#Fix Text: Document the files with the suid bit set, or unset the suid 
