...
#|    1.0   |   Initial Script      | Vincent C. Passaro | 30-MAR-2012|
#|	    |   Creation	    |                    |            |
#|__________|_______________________|____________________|____________|
...
if [ $FIREFOX64 -eq 1 ]
  then
	...
	GENCONFIGNAM64=$( cat $FIREFOX64 | grep 'pref("general.config.filename", "mozilla-aqueduct-pci.cfg");' | wc -l )
	CONFIGDIR64=$( find /usr/lib64/ -name all-redhat.js -print | grep firefox | cut -d '/' -f 1-4 )
	AQUEDUCT64=$( find /usr/lib64/ -name mozilla-aqueduct-pci.cfg -print | grep firefox | wc -l )
  ...
  #Begin Lockdown
			...
			if [ $GENCONFIGVAL64 -eq 0 ]
				then
					echo "//" >> $FIREFOX64
					echo "//Adding General Config Value for Aqueduct Mozilla Firefox PCI Lockdown" >> $FIREFOX64
					echo "//" >> $FIREFOX64
					echo 'pref("general.config.obscure_value", 0);' >> $FIREFOX64
					echo 'pref("general.config.filename", "mozilla-aqueduct-pci.cfg");' >> $FIREFOX64
			fi
		#Now we make sure the mozilla-aqueduct.cfg file is there.  If not, we create it and set proper permission so nobody goof's with it. 
			if [ $AQUEDUCT64 -eq 0 ]
				then
					...
					chown root:root mozilla-aqueduct-pci.cfg
					chmod 644 mozilla-aqueduct-pci.cfg

					cp mozilla-aqueduct-pci.cfg $CONFIGDIR64
			fi
  else					
...
if [ $FIREFOX32 -eq 1 ]
  then
		...
		GENCONFIGNAM32=$( cat $FIREFOX32 | grep 'pref("general.config.filename", "mozilla-aqueduct-pci.cfg");' | wc -l )
		CONFIGDIR32=$( find /usr/lib/ -name all-redhat.js -print | grep firefox | cut -d '/' -f 1-4 )
		AQUEDUCT32=$( find /usr/lib/ -name mozilla-aqueduct-pci.cfg -print | grep firefox | wc -l  )
  
			if [ $GENCONFIGVAL32 -eq 0 ]
				then
					echo "//" >> $FIREFOX32
					echo "//Adding General Config Value for Aqueduct Mozilla Firefox DISA pci Lockdown" >> $FIREFOX32
					echo "//" >> $FIREFOX32
					echo 'pref("general.config.obscure_value", 0);' >> $FIREFOX32
					echo 'pref("general.config.filename", "mozilla-aqueduct-pci.cfg");' >> $FIREFOX32
			fi
		#Now we make sure the mozilla-aqueduct.cfg file is there.  If not, we create it and set proper permission so nobody goof's with it. 
			if [ $AQUEDUCT32 -eq 0 ]
				then
					...
					cp mozilla-aqueduct-pci.cfg $CONFIGDIR32
			fi
  else					
